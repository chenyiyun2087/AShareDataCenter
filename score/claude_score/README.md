...
> 说明：以下规则与 `scripts/etl/dws/scoring.py` 的 SQL 计算逻辑保持一致；
> 文档侧重主干打分区间，代码中还包含缺失值与边界值兜底（如 `NULL` 多数按 0 分或基准分处理）。
...
| 指标 | 权重 | 评分规则 |
|------|------|----------|
| **5日涨幅** | 3分 | >10%: 3分 \| >5%: 2分 \| >0%: 1分 |
| **20日涨幅** | 2分 | >15%: 2分 \| >5%: 1分 (与MTM重叠，权重降低) |
| **60日涨幅** | 3分 | >30%: 3分 \| >10%: 2分 \| >0%: 1分 |
| **量比** | 4分 | >1.5: 4分 \| >1.2: 3分 \| >1.0: 2分 \| 其他: 1分 |
| **换手率** | 4分 | >10%: 4分 \| >5%: 3分 \| >2%: 2分 \| 其他: 1分 |
| **MTM动量指标** | 5分 | >1.0: 5分 \| >0.5: 4分 \| >0.2: 3分 \| >0: 2分 \| >-0.5: 1分 |
| **MTMMA交叉** | 4分 | 金叉+多头: 4分 \| 金叉: 3分 \| 双多头区: 2分 \| 接近零轴: 1分 |
...
| 指标 | 权重 | 评分规则 |
|------|------|----------|
| **MACD** | 4分 | 金叉(DIF>DEA & MACD>0): 4分 \| 红柱(MACD>0): 2分 \| 多头(DIF>DEA): 1分 |
| **KDJ (J值)** | 3分 | 超卖(<20): 3分 \| 强势区(40-60): 2分 \| 超买(>80): 0分 |
| **RSI (6日)** | 3分 | 超卖(<30): 3分 \| 强势区(40-60): 2分 \| 超买(>70): 0分 |
| **CCI (顺势)** | 3分 | 强势突破(>100): 3分 \| 多头区(>0): 2分 \| 超卖(<-100): 2分 |
| **BIAS (乖离率)** | 2分 | 超卖(<-3%): 2分 \| 正常区: 1分 \| 过热(>5%): 0分 |
...
| 指标 | 权重 | 评分规则 |
|------|------|----------|
| **特大单** | 5分 | >1亿: 5分 \| >5千万: 4分 \| >1千万: 2分 \| >0: 1分 |
| **大单** | 3分 | >5千万: 3分 \| >2千万: 2分 \| >0: 1分 |
| **融资融券** | 2分 | 净买入比例 `(买入-偿还)/余额` >2%: 2分 \| >0.5%: 1.5分 \| >0: 1分 \| 净卖出: 0.5分 \| 非融资标的: 1分 |
...
## 规则一致性校验（README vs ETL SQL）

- 满分结构一致：动量25 + 价值20 + 质量20 + 技术15 + 资金10 + 筹码10 = 100。
- 六大维度的核心阈值与权重一致，SQL 实际使用同一套分段规则写入 `dws_*_score` 表。
- 代码层补充了文档未展开的兜底分策略（例如部分技术指标 `NULL` 给 1 分基准分），不改变主规则方向。
